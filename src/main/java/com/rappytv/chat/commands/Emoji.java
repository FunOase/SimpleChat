...
public class Emoji implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String prefix, String[] args) {
        if(!(sender instanceof Player)) {
            sender.sendMessage(ChatPlugin.prefix + "§cDie Konsole darf diesen Befehl nicht ausführen!");
            return false;
        }
        Player player = (Player) sender;

        if(!player.hasPermission("chat.emojis.toggle")) {
            player.sendMessage(ChatPlugin.prefix + "§cDazu hast du keine Rechte!");
            return false;
        }
        ...
        player.sendMessage(ChatPlugin.prefix + "Emojis sind nun " + text + "§7!");
        return true;
    }
}
